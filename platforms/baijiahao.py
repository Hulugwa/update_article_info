...
def get_id(url):
    try:
        response = BaseFunctions.requests().get(url, verify=False, timeout=DefaultValues.timeout, proxies=DefaultValues.proxies)
...
def baijiahao(url):
    ...
    try:
        ...
        response = BaseFunctions.requests().get(request_url, verify=False, proxies=DefaultValues.proxies, headers=headers)
